...
Every cluster ships with **Kong Gateway (Gateway API)** as the sole ingress controller (Traefik is disabled, Flannel is replaced when Calico is selected). Kong installs Gateway API CRDs v1.5.1, Kong Ingress Controller (v3.5.6), a `GatewayClass`, and a `Gateway`. httpbin is exposed via `HTTPRoute`.
...
|-----------|---------|-------------|
| **Calico CNI** | Yes | Replaces Flannel. Enables Kubernetes NetworkPolicy support. |
