...
* `helpers.sh`: the Cilium images are now pulled for the node architecture only (`docker pull --platform linux/<arch>`, derived from `uname`). On Docker Desktop / Rancher Desktop with the containerd image store a bare pull keeps the full multi-arch manifest, and `k3d image import` of such an image failed on every node with `ctr/tr: content digest sha256:...: not found`. A single-platform image exports and imports cleanly; this is a no-op on the classic image store.
