...
* **Kong AI Gateway per-user token metrics now come from OpenTelemetry instead of a custom exporter.** `examples/kong-ai-gateway` drops the Python `ai-metrics-exporter` (an `http-log` sink that parsed each provider's response body) and the `ai-force-nostream` hack. Kong's `opentelemetry` plugin now ships access logs — carrying the AI plugin's `log_statistics` token counts plus `provider`/`model`/`consumer` attributes (`custom_attributes_by_lua`) — to a new **OTel Collector** (`otel-collector.yaml`) whose `sum` connector emits the same `ai_llm_*` Prometheus metrics the Grafana dashboard already uses. Cost is computed by Kong from per-model `input_cost`/`output_cost` rates. Result: no bespoke code, streaming keeps working, and traces still flow to Tempo. Removed `ai-metrics-exporter.yaml`, `kong-http-log-plugin.yaml`, `kong-ai-nostream-plugin.yaml`, `kong-ai-plugins.yaml`; the prometheus scrape target and Kuma HTTP/1.1 patch were repointed to the collector.
...
* `examples/openbao/setup.sh`: wait for the `openbao-0` pod to be created before `kubectl wait` runs on it. `helm install` returns before the StatefulSet controller creates the pod, and `kubectl wait` errors out immediately with `pods "openbao-0" not found` if the object does not exist yet — a race that triggers on a busy cluster (e.g. when chained from the Kong AI Gateway complete setup). Surfaced by `complete-setup.sh`.
* Cilium is now installed with `cni.exclusive: false` so the Kuma service-mesh CNI can chain onto it. With the default `cni.exclusive: true` Cilium continuously rewrites `/etc/cni/net.d/05-cilium.conflist` and strips Kuma's appended `kuma-cni` plugin, so `kuma-cni-node` crash-looped, the `NodeReadiness:NoSchedule` taint was never cleared, and meshed pods failed their `kuma-validation` init container (transparent proxy never installed). Needed together with the Kuma conflist-detection fix for the kuma-mesh and Kong AI Gateway Kuma paths on the now-default Cilium CNI.
